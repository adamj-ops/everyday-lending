...
export const loanSchema = z.object({
  ...
  borrowerId: z.coerce.number().int().min(1, 'Borrower is required'),
  propertyId: z.coerce.number().int().min(1, 'Property is required'),
  ...
  monthlyPayment: z.coerce.number().optional(), // Can be auto-calculated
  originationDate: z.string().min(1, 'Origination date is required'),
  maturityDate: z.string().optional(), // Can be auto-calculated
  status: z.enum(['active', 'paid_off', 'defaulted', 'foreclosed', 'cancelled']).optional().default('active'),
  currentBalance: z.coerce.number().optional(), // Defaults to loan amount
  principalPaid: z.coerce.number().optional().default(0),
  interestPaid: z.coerce.number().optional().default(0),
  feesPaid: z.coerce.number().optional().default(0),
  lateFeesPaid: z.coerce.number().optional().default(0),
